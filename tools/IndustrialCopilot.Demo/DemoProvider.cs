...
internal sealed class DemoProvider : ILlmProvider
{
    public Task<ToolCompletionResponse> CompleteWithToolsAsync(CompletionRequest request,IReadOnlyList<ToolDefinition> tools,CancellationToken ct)
    {
        ...
        if(system.Contains("You are the Symptom Matcher",StringComparison.Ordinal))
        {
            ...
            if(!symptom.Contains("vibration",StringComparison.OrdinalIgnoreCase) && !symptom.Contains("اهتزاز",StringComparison.Ordinal))
                return Output("""{"outcome":"InsufficientEvidence"}""");
            if(!request.Messages.Any(m=>m.Role==LlmRole.Tool))
            {
                using var args=JsonDocument.Parse("""{"candidate":0,"query":"vibration pump seal"}""");
                return Task.FromResult(new ToolCompletionResponse("",[new("demo-retrieval","retrieve_evidence",args.RootElement)],new(10,5,15)));
            }
            // No match without retrieved evidence. The actual agent validates and resolves e0.
            if(request.Messages.Last().Content=="[]")return Output("""{"outcome":"InsufficientEvidence"}""");
            ...
            return Output(JsonSerializer.Serialize(new{outcome="Success",candidate=0,symptoms=new[]{new{description,evidence=new[]{"e0"}}}}));
        }
        if(system.Contains("You are the Diagnostic & Safety Planner",StringComparison.Ordinal))
            return Output("""{"outcome":"Success","steps":[{"order":1,"instruction":"Check vibration","evidence":["e0"]}],"prerequisites":[]}""");
        if(system.Contains("You are the Work Order Generator",StringComparison.Ordinal))
            return Output("""{"outcome":"Success","description":"Inspect isolated pump","actions":[{"order":1,"instruction":"Inspect seal","evidence":["e0"]}]}""");
